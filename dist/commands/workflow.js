...
const dooray_1 = require("../services/dooray");
...
exports.workflowCommand = new commander_1.Command('workflow')
    ...
    try {
        logger_1.logger.info(`🚀 Starting workflow for task: ${projectId}/${taskId}`);
        // 진행률 콜백 설정
        ...
        // 워크플로우 실행
        const result = await (0, workflow_1.executeTaskWorkflow)(projectId, taskId, workflowOptions, progressCallback);
        // 결과 표시
        console.log('\n'); // 진행률 바 다음 줄로
        ...
        logger_1.logger.error(`❌ Workflow failed: ${errorMessage}`);
        // 상세 오류 정보 표시
        if (error instanceof Error && error.stack && process.env['DEBUG']) {
            logger_1.logger.debug('Error stack:', error.stack);
        }
        process.exit(1);
    }
});
 ...
 * 빠른 검증 수행
 */
async function performQuickValidation(options) {
    logger_1.logger.info('🔍 Performing connection validation...');
    const validations = [];
    // Dooray 연결 확인
    validations.push((0, dooray_1.validateDoorayConnection)().then(valid => {
        if (!valid)
            throw new Error('Dooray API connection failed');
        logger_1.logger.success('✅ Dooray API connected');
    }));
    // GitHub 연결 확인 (PR 생성이 활성화된 경우)
    if (!options.noPr) {
        validations.push((0, github_1.validateGitHubConnection)().then(valid => {
            if (!valid)
                throw new Error('GitHub API connection failed');
            logger_1.logger.success('✅ GitHub API connected');
        }));
    }
    // Git 저장소 확인 (커밋이 활성화된 경우)
    if (!options.noCommit) {
        validations.push((0, git_1.validateGitRepository)().then(valid => {
            if (!valid)
                throw new Error('Not a valid Git repository');
            logger_1.logger.success('✅ Git repository validated');
        }));
    }
    try {
 ...
 * 워크플로우 결과 표시
 */
function displayWorkflowResult(result, options) {
    if (result.success) {
        logger_1.logger.success('🎉 Workflow completed successfully!');
        console.log('\n📋 Workflow Summary:');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        if (result.task) {
...
// 사용 예제를 위한 도움말 확장
exports.workflowCommand.addHelpText('after', `
...
`);
exports.default = exports.workflowCommand;
