...
const defaultConfig: DoorayAIConfig = {
  ai: {
    provider: 'claude',
...
async function loadConfig(): Promise<DoorayAIConfig> {
  try {
    ...
    logger.warn('설정 파일 로드 중 오류 발생, 기본 설정을 사용합니다.');
  }
  return defaultConfig;
}

async function saveConfig(config: DoorayAIConfig): Promise<void> {
...
configCommand
  ...
  .action(async (options) => {
    try {
      if (!options.force && await fs.pathExists(CONFIG_FILE)) {
        logger.error('설정 파일이 이미 존재합니다. --force 옵션을 사용하여 덮어쓸 수 있습니다.');
        return;
      }
      
      await saveConfig(defaultConfig);
      logger.success('기본 설정 파일이 생성되었습니다.');
      logger.info(`설정 파일 위치: ${CONFIG_FILE}`);
    } catch (error) {
      logger.error('설정 파일 생성 중 오류가 발생했습니다:', error);
    }
  });
...
configCommand
  ...
  .action(async (key) => {
    try {
      ...
      if (value === undefined) {
        logger.error(`설정 키 '${key}'를 찾을 수 없습니다.`);
        return;
      }
      
      console.log(`${key} = ${JSON.stringify(value, null, 2)}`);
    } catch (error) {
      logger.error('설정 조회 중 오류가 발생했습니다:', error);
    }
  });
...
configCommand
  ...
  .action(async (key, value) => {
    try {
      ...
      logger.success(`설정 '${key}'가 '${value}'로 변경되었습니다.`);
    } catch (error) {
      logger.error('설정 변경 중 오류가 발생했습니다:', error);
    }
  });
...
configCommand
  ...
  .action(async () => {
    try {
      ...
      if (errors.length === 0) {
        logger.success('설정 파일이 유효합니다.');
      } else {
        logger.error('설정 파일에 다음 오류가 있습니다:');
...
configCommand
  ...
  .action(async (options) => {
    try {
      if (!await fs.pathExists(CONFIG_FILE)) {
        logger.error('백업할 설정 파일이 존재하지 않습니다.');
        return;
      }
      
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const backupPath = options.output || path.join(CONFIG_DIR, `config-backup-${timestamp}.json`);
      
      await fs.copy(CONFIG_FILE, backupPath);
      logger.success(`설정 파일이 백업되었습니다: ${backupPath}`);
    } catch (error) {
      logger.error('설정 백업 중 오류가 발생했습니다:', error);
    }
  });
...
configCommand
  ...
  .action(async (backupPath) => {
    try {
      ...
      await fs.copy(backupPath, CONFIG_FILE);
      logger.success('설정이 복원되었습니다.');
    } catch (error) {
      logger.error('설정 복원 중 오류가 발생했습니다:', error);
    }
  });
...
configCommand
  ...
  const answers = await inquirer.default.prompt([
    ...
    },
    {
      ...
      logger.error('설정 마법사 실행 중 오류가 발생했습니다:', error);
    }
  });
