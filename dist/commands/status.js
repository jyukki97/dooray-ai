...
async function getProjectInfo() {
    ...
    try {
        ...
        logger_1.logger.debug('프로젝트 정보 수집 중 오류:', error);
    }
    return projectInfo;
}
async function getGitInfo() {
    ...
    try {
        // Git 저장소인지 확인
        if (!await fs_extra_1.default.pathExists(path_1.default.join(process.cwd(), '.git'))) {
            gitInfo.status = 'Git 저장소 아님';
            return gitInfo;
        }
        // 현재 브랜치
        gitInfo.branch = (0, child_process_1.execSync)('git branch --show-current', { encoding: 'utf8' }).trim();
        // Git 상태
        ...
        // 커밋 수
        try {
            const commitCount = (0, child_process_1.execSync)('git rev-list --count HEAD', { encoding: 'utf8' });
        ...
        // 원격 저장소
        try {
            gitInfo.remote = (0, child_process_1.execSync)('git remote get-url origin', { encoding: 'utf8' }).trim();
        }
        catch {
        ...
        logger_1.logger.debug('Git 정보 수집 중 오류:', error);
    }
    return gitInfo;
}
async function getConfigInfo() {
    const configPaths = [
        path_1.default.join(process.cwd(), '.dooray-ai', 'config.json'),
        path_1.default.join(require('os').homedir(), '.dooray-ai', 'config.json')
    ];
...
async function getDependencyInfo() {
    ...
    try {
        if (await fs_extra_1.default.pathExists(packageJsonPath)) {
            ...
            // 취약점 확인 (npm audit)
            try {
                const auditResult = (0, child_process_1.execSync)('npm audit --json', { encoding: 'utf8' });
            ...
            try {
                (0, child_process_1.execSync)('npm outdated --json', { encoding: 'utf8' });
            }
            catch (error) {
                if (error.stdout) {
        ...
        logger_1.logger.debug('종속성 정보 수집 중 오류:', error);
    }
    return depInfo;
}
async function getFileInfo() {
    ...
    try {
        ...
        logger_1.logger.debug('파일 정보 수집 중 오류:', error);
    }
    return fileInfo;
}
...
function calculateHealthScore(status) {
    ...
    if (!status.git.hasUncommitted)
        ...
        recommendations.push('커밋되지 않은 변경사항을 커밋하세요');
    if (status.git.remote !== '설정되지 않음')
        score += 10;
    else
        recommendations.push('원격 저장소를 설정하세요');
    // 설정 상태 (25점)
    if (status.config.exists)
        score += 15;
    else
        recommendations.push('Dooray AI 설정을 초기화하세요 (dooray-ai config init)');
    if (status.config.valid)
        score += 10;
    else
        recommendations.push('설정 파일을 검증하세요 (dooray-ai config validate)');
    // 종속성 상태 (25점)
    if (status.dependencies.vulnerabilities === 0)
        score += 15;
    else
        recommendations.push('보안 취약점을 해결하세요 (npm audit fix)');
    if (status.dependencies.outdated === 0)
        score += 10;
    else
        recommendations.push('오래된 패키지를 업데이트하세요 (npm update)');
    // 코드 품질 (25점)
    ...
    if (codeRatio > 0.8)
        ...
        score += 10;
    else if (codeRatio > 0.2)
        score += 5;
    else
        recommendations.push('TypeScript 사용을 늘려보세요');
    ...
    if (testRatio > 0.2)
        ...
        recommendations.push('테스트 커버리지를 늘려보세요');
