...
const dooray_1 = require("../dooray");
const github_1 = require("../github");
const git_1 = require("../git");
const code_generator_1 = require("../ai/code-generator");
...
class TaskWorkflowEngine {
     ...
     * 태스크 워크플로우 실행
     */
    async executeTaskWorkflow(input) {
        const startTime = Date.now();
        try {
            logger_1.logger.info(`Starting workflow for task: ${input.projectId}/${input.taskId}`);
            // 설정 로드 및 병합
            ...
            try {
                ...
                const task = await this.executeStep('fetch-task', async () => {
                    const doorayClient = await (0, dooray_1.getDoorayClient)();
                ...
                // Dry run 체크
                if (options.dryRun) {
                    logger_1.logger.info('Dry run mode - skipping code generation and git operations');
                    result.success = true;
                ...
                if (config.autoCommit) {
                    const gitResult = await this.executeStep('git-workflow', async () => {
                        const gitClient = await (0, git_1.getGitClient)();
                        const filePaths = generatedFiles.map(f => f.path);
                        return await gitClient.executeTaskWorkflow(input.taskId, task.subject, filePaths, {
                ...
                if (config.createPR && result.gitResult?.branchName) {
                    const pullRequest = await this.executeStep('create-pr', async () => {
                        const githubClient = await (0, github_1.getGitHubClient)();
                        return await githubClient.createTaskPullRequest(input.taskId, task.subject, task.body, result.gitResult.branchName, {
                ...
                if (config.linkToTask && result.pullRequest) {
                    await this.executeStep('update-task', async () => {
                        ...
                        return await doorayClient.updateTask(input.projectId, input.taskId, {
                            body: updateBody,
                            status: 'working'
                        });
                    });
                ...
                result.success = true;
                logger_1.logger.success(`Workflow completed successfully for task: ${input.taskId}`);
            }
            catch (error) {
                result.error = error;
                logger_1.logger.error(`Workflow failed: ${error}`);
                // 실패 시 정리
            ...
            logger_1.logger.error(`Task workflow failed: ${error}`);
            throw error;
        }
    }
    /**
    ...
    initializeSteps(options) {
        ...
        if (options.dryRun) {
            // Dry run에서는 일부 단계 스킵
            this.steps = this.steps.filter(step => ['validation', 'fetch-task', 'analyze-task', 'prepare-context'].includes(step.id));
        }
    }
    /**
     * 워크플로우 단계 실행
    ...
    async executeStep(stepId, executor) {
        ...
        this.updateProgress();
        try {
            logger_1.logger.info(`Executing step: ${step.name}`);
            const result = await executor();
            step.status = 'completed';
            step.endTime = new Date();
            step.result = result;
            logger_1.logger.success(`Step completed: ${step.name}`);
            this.updateProgress();
            return result;
        }
        catch (error) {
            ...
            step.endTime = new Date();
            step.error = error;
            logger_1.logger.error(`Step failed: ${step.name} - ${error}`);
            this.updateProgress();
            throw error;
        }
    }
    ...
    async loadConfig(overrides) {
        try {
            ...
            logger_1.logger.warn('Failed to load config, using defaults');
     ...
     * 워크플로우 입력 유효성 검증
     */
    async validateWorkflowInput(input, config) {
        const errors = [];
        const warnings = [];
        // 필수 필드 검증
        if (!input.projectId)
        ...
        if (!input.options?.skipValidation) {
            try {
                ...
                if (!doorayValid)
                    errors.push('Dooray API connection failed');
            }
            catch (error) {
     ...
     * 워크플로우 컨텍스트 생성
     */
    async createWorkflowContext(task, analysis, config, options) {
...
Task: ${task.id}`;
    ...
    async generateCode(context) {
        ...
        logger_1.logger.info('Generating code based on task analysis...');
        try {
            ...
            if (analysis.estimatedComplexity === 'high' || analysis.technicalSpecs.length > 3) {
                // 복잡한 태스크는 프로젝트 생성
                result = await code_generator_1.codeGenerator.generateProject(prompt, 'web-app', // 기본값, 추후 분석 기반으로 결정
                generationOptions);
            }
            else {
                // 단순한 태스크는 코드 생성
                result = await code_generator_1.codeGenerator.generateCode(prompt, generationOptions);
            }
            ...
            logger_1.logger.success(`Generated ${files.length} files`);
            return files;
        }
        catch (error) {
            logger_1.logger.error('Code generation failed');
            throw error;
        }
    }
    /**
     * 코드 생성 프롬프트 생성
     */
    generateCodePrompt(task, analysis) {
...
${analysis.technicalSpecs.map((spec) => `- ${spec}`).join('\n')}
...
Please implement the solution following these requirements and best practices.`;
     ...
     * 생성된 파일 저장
     */
    async saveGeneratedFiles(files, workingDirectory) {
        logger_1.logger.info(`Saving ${files.length} generated files...`);
        for (const file of files) {
            ...
            logger_1.logger.debug(`Saved file: ${file.path}`);
        }
        logger_1.logger.success('All files saved successfully');
    }
    /**
     * 파일 타입 결정
     */
    determineFileType(filePath, declaredType) {
        if (declaredType) {
            return declaredType;
        }
     ...
     * 실패 시 정리
     */
    async cleanupOnFailure(result) {
        logger_1.logger.info('Cleaning up after workflow failure...');
        try {
            // Git 브랜치 정리
            if (result.gitResult?.branchName) {
                ...
                try {
                    ...
                    logger_1.logger.info(`Cleaned up branch: ${result.gitResult.branchName}`);
                }
                catch (error) {
                    logger_1.logger.warn(`Failed to cleanup branch: ${error}`);
                }
            }
            // 생성된 파일 정리 (선택적)
            ...
            logger_1.logger.warn(`Cleanup failed: ${error}`);
        }
    }
}
exports.TaskWorkflowEngine = TaskWorkflowEngine;
